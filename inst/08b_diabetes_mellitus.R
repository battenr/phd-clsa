...
# Note: WIP. Need to recode diabetes_melltius based on 
# other variables because it was based on "type I", "type II", etc 
# so some of the NAs should actually be no 
...
formula = as.formula("diabetes_mellitus ~ bzd + age + region + marital_status + smoke + household_income")
...
X2 <- model.matrix(
  ~bzd + age + region + marital_status + smoke + household_income,
  data = data.frame(X1)
)
...
mod_refit <- svyglm(
  depression ~ bzd + centered_age + sex + region + marital_status + smoke + 
    household_income + bzd*centered_age, 
  design = design_analytic,
  family = stats::binomial(link = "logit")
)
...
X2 <- model.matrix(
  ~ bzd + centered_age + sex + region + marital_status + smoke + 
    household_income + bzd*centered_age,
  data = data.frame(X1)
)

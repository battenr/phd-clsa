...
df_regression <- format_data(outcome = "stroke")
...
# Sex is a known confounder so going to include anyways (associated with both BZD and Stroke)

check_confounding(fully_adjusted =   
                    reformulate(response = outcome, 
                                termlabels = c(keep_vars$var, "sex")
                    ),
                  partially_adjusted = 
                    reformulate(response = outcome, 
...
table_matrix <- svytable(~stroke + bzd + sex, 
                         design = design_analytic)  %>% 
  as.matrix() 
...
table_matrix <- svytable(~stroke + bzd + income, 
                         design = design_analytic) %>% 
  as.matrix() 
...
table_matrix <- svytable(~stroke + bzd + smoke, 
                         design = design_analytic) %>% 
  as.matrix() 
